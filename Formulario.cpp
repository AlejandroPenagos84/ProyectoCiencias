    ...
    for (int i = 0; i < empleado.numHijos; ++i) {
        ...
        do {
            ...
            if (!std::all_of(hijo.fechaNacimiento.begin(), hijo.fechaNacimiento.end(), ::isdigit)) {
                std::cerr << "Error: La fecha debe contener solo números." << std::endl;
            }
    ...
    try {
        ...
        do {
            ...
            if (!std::all_of(empleado.tipoIdentificacion.begin(), empleado.tipoIdentificacion.end(), ::isalpha)) {
                std::cerr << "Error: El tipo de identificación debe contener solo letras." << std::endl;
            }
        ...
        do {
            ...
            if (!std::all_of(empleado.numIdentificacion.begin(), empleado.numIdentificacion.end(), ::isdigit)) {
                std::cerr << "Error: El número de identificación debe contener solo números." << std::endl;
            }
        ...
        do {
            ...
            if (std::cin.fail()) {
                ...
                std::cerr << "Error: El teléfono celular debe ser un número válido." << std::endl;
            }
        } while (std::cin.fail());
        ...
        do {
            ...
            if (std::cin.fail()) {
                std::cin.clear();
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                std::cerr << "Error: El teléfono fijo debe ser un número válido." << std::endl;
            }
        } while (std::cin.fail());
        ...
        do {
            ...
            // Puedes implementar una validación más compleja para un formato de email específico si es necesario
        ...
        do {
            ...
            if (!std::all_of(empleado.fechaNacimiento.begin(), empleado.fechaNacimiento.end(), ::isdigit)) {
                std::cerr << "Error: La fecha de nacimiento debe contener solo números." << std::endl;
            }
        ...
        do {
            ...
            if (!std::all_of(empleado.paisNacimiento.begin(), empleado.paisNacimiento.end(), ::isalpha)) {
                std::cerr << "Error: El país de nacimiento debe contener solo letras." << std::endl;
            }
        ...
        do {
            ...
            // Puedes ajustar la validación según tus requisitos específicos
        ...
        // ... Continuar con el mismo patrón para otras secciones ...
        ...
        if (empleado.tieneHijos == 'S' || empleado.tieneHijos == 's') {
            do {
                ...
                if (std::cin.fail()) {
                    std::cin.clear();
                    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                    std::cerr << "Error: El número de hijos debe ser un número válido." << std::endl;
                }
            } while (std::cin.fail());
        ...
        std::cerr << "Error desconocido durante la captura de información del empleado." << std::endl;
        return Empleado();
    }
}
...
Sucursal Formulario::capturarInformacionSucursal() {
    ...
    try {
        ...
        // Puedes continuar con el mismo patrón para otras secciones si las tienes

        return sucursal;

    } catch (const std::invalid_argument& e) {
        std::cerr << "Error: " << e.what() << std::endl;
        // función devuelve un objeto Sucursal vacío.
        return Sucursal();
    } catch (...) {
        std::cerr << "Error desconocido durante la captura de información de la sucursal." << std::endl;
        // función devuelve un objeto Sucursal vacío.
        return Sucursal();
    }
}
